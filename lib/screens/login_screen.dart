...
class _LoginScreenState extends State<LoginScreen> {
  ...
  // void handleLoginSuccess(Usuario usuario) {
  //   if (usuario.rol == 'tecnico') {
  //     Navigator.pushReplacement(
  //         context, HomeTechnicianScreen(usuario: usuario) as Route<Object?>);
    ...
    if (_formKey.currentState?.validate() ?? false) {
      ...
      try {
        final response = await http.post(
          Uri.parse(
              'https://backendnodeproyectomesaservicio.onrender.com/api/auth/login'), // Cambia la URL por la de tu backend
        ...
        if (response.statusCode == 200 && response.body.isNotEmpty) {
          ...
          final userfound = data['dataUser']['user'];
          print('usuario: $userfound');
          final user = Usuario.fromJson(userfound);

          final String token = data['dataUser']['token'];
          print(token);
          await _saveToken(token); // Guardar el token

          if (userfound['rol'] == 'tecnico') {
            print(user);
            Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (context) => HomeTechnicianScreen(
                        usuario: user,
                      )),
            );
          } else if (userfound['rol'] == 'funcionario') {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => HomeOfficerScreen()),
            );
          }
        } else {
                        ...
                        children: [
                          ...
                          ElevatedButton(
                            ...
                            style: ElevatedButton.styleFrom(
                              ...
                              backgroundColor:
                                  const Color.fromRGBO(0, 50, 77, 1),
